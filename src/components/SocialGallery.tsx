...
export const SocialGallery: React.FC = () => {
  ...
  const activePostEmbedUrl = useMemo(() => {
    if (!activeItem) return null;
    let url = activeItem.postUrl || '';
    if (!url && activeItem.image && activeItem.image.includes('eeinstagram.com/images/')) {
      const match = activeItem.image.match(/\/images\/([^/]+)/);
      if (match) {
    ...
    try {
      ...
      const pIndex = parts.findIndex((p: string) => p === 'p' || p === 'reel' || p === 'tv' || p === 'reels');
      if (pIndex !== -1 && pIndex + 1 < parts.length) {
        const shortcode = parts[pIndex + 1];
    ...
    <section className="py-16 md:py-24 px-4 sm:px-6 md:px-8 bg-gradient-to-b from-[#FAF8FA] to-[#fff7fa]">
      ...
      <Dialog open={!!activeItem} onOpenChange={(open) => !open && setActiveItem(null)}>
        <DialogContent className={cn(
          "rounded-2xl overflow-hidden p-0 border-0 shadow-2xl bg-white max-h-[90vh] overflow-y-auto",
          activePostEmbedUrl ? "max-w-[90vw] sm:max-w-[450px]" : "max-w-[90vw] md:max-w-md bg-transparent shadow-none"
        )}>
          {activeItem && (
            activePostEmbedUrl ? (
              <div className="relative w-full bg-white flex flex-col justify-center items-center min-h-[500px]">
                <iframe
                  src={activePostEmbedUrl}
