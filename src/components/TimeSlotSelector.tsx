...
const TimeSlotSelector = ({
  ...
  // Dynamic maximum date calculation based on current date
  const getMaximumDate = (): Date => {
    const currentDate = new Date();
    const currentYear = currentDate.getFullYear();
    const currentMonth = currentDate.getMonth(); // 0-11 (January = 0, December = 11)
    const currentDay = currentDate.getDate();
    
    // Check if we're past June 30th (half year completed)
    const isSecondHalfOfYear = currentMonth > 5 || (currentMonth === 5 && currentDay >= 30);
    
    if (isSecondHalfOfYear) {
      // After June 30th: Allow booking until June 30th of NEXT year
      return new Date(currentYear + 1, 5, 30); // June 30 of next year
    } else {
      // Before June 30th: Allow booking until December 31st of CURRENT year
      return new Date(currentYear, 11, 31); // December 31 of current year
    }
  };

  const endOfYear = getMaximumDate();

  // Fetch holidays for the current year AND next year if applicable
  useEffect(() => {
    const fetchHolidays = async () => {
      try {
        ...
        const currentYearResponse = await holidayService.getHolidaysForYear(currentYear);
        let allHolidays: Holiday[] = [];
        
        if (currentYearResponse.success && currentYearResponse.data.length > 0) {
          allHolidays = [...currentYearResponse.data];
        } else {
          allHolidays = [...calculateFallbackHolidays(currentYear)];
        }
        
        // If max year is different (next year is unlocked), fetch next year's holidays too
        if (maxYear > currentYear) {
          try {
            const nextYearResponse = await holidayService.getHolidaysForYear(maxYear);
            if (nextYearResponse.success && nextYearResponse.data.length > 0) {
              allHolidays = [...allHolidays, ...nextYearResponse.data];
            } else {
              allHolidays = [...allHolidays, ...calculateFallbackHolidays(maxYear)];
            }
          } catch (error) {
  ...
  const isDateDisabled = (date: Date): boolean => {
    ...
    // Check if date is after the calculated maximum date
    const maxDate = getMaximumDate();
    if (date > maxDate) {
      return true;
    }
          ...
          <Popover.Content side="bottom" align="start" className="z-[9999] bg-white rounded-lg shadow-lg p-2 mt-2">
            <div className="space-y-2">
              ...
              {(() => {
                const maxDate = getMaximumDate();
                const currentDate = new Date();
                const isSecondHalf = currentDate.getMonth() > 5 || (currentDate.getMonth() === 5 && currentDate.getDate() >= 30);
                
                return (
                  <div className="border-b pb-2 mb-2">
                    <p className="text-xs text-blue-600 font-medium">
                      {isSecondHalf ? (
                        <>📅 Booking available until {format(maxDate, 'MMMM dd, yyyy')} (Next year's first half unlocked!)</>
                      ) : (
                        <>📅 Booking available until {format(maxDate, 'MMMM dd, yyyy')}</>
                      )}
                    </p>
                  </div>
                );
