...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const checkAuthStatus = useCallback(async (force = false) => {
    ...
    // Never allow concurrent auth checks.
    if (isCheckingAuthRef.current) {
      return;
    }

    // Guard against request storms from repeated events/renders.
    if (!force && now - lastAuthCheckRef.current < 1500) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      ...
      if (event.key === 'isAuthenticated' || event.key === 'token' || event.key === null) {
