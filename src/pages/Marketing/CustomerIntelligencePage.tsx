    ...
    try {
      ...
      if (res?.success) {
        ...
        if (res.customers?.length > 0) {
          loadProfile(res.customers[0].visitorId);
        }
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {loading ? (
              ...
              filteredCustomers.map((c) => {
                ...
                return (
                  <div
                    key={c.visitorId}
                    onClick={() => loadProfile(c.visitorId)}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-800 text-rose-300">
                        {c.interestLevel || 'Low'} ({c.interestScore || 5})
                      </span>
                    </div>

                    <div className="flex items-center justify-between text-[11px] text-slate-400 font-mono">
                      <span>{c.visitorId.slice(0, 16)}...</span>
        ...
        {/* Right: Detailed Customer Profile (7 cols) */}
        <div className="lg:col-span-7">
          {selectedProfile ? (
            <Card className="bg-slate-900/70 border-slate-800/80 shadow-xl sticky top-24">
              <CardHeader className="border-b border-slate-800/80 pb-4">
                <div className="flex items-start justify-between">
                  ...
                  <Button
                    ...
                    className="bg-rose-600 hover:bg-rose-500 text-white text-xs font-semibold h-8 rounded-xl shadow-md shadow-rose-950/40 flex items-center gap-1.5"
                  >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    ...
                    <div className="p-3 bg-slate-800/30 rounded-xl border border-slate-700/40 flex items-center justify-between">
                      ...
                      <span className="font-semibold text-slate-200">{selectedProfile.behavior.preferredDevice}</span>
                    </div>
                    <div className="p-3 bg-slate-800/30 rounded-xl border border-slate-700/40 flex items-center justify-between">
                      <span className="text-slate-400">Traffic Acquisition:</span>
                      <span className="font-semibold text-slate-200">{selectedProfile.behavior.trafficSource}</span>
                    </div>
                  </div>
                </div>
