...
export const MarketingLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-100 font-sans selection:bg-rose-500/30 selection:text-rose-200">
      {/* Sidebar */}
      <aside
        className={cn(
          "relative flex flex-col border-r border-slate-800/80 bg-slate-900/90 backdrop-blur-xl transition-all duration-300 z-40 shrink-0",
          collapsed ? "w-20" : "w-64"
        )}
      >
        {/* Brand Header */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800/80">
          ...
          <Button
            ...
            className="h-8 w-8 text-slate-400 hover:text-slate-100 hover:bg-slate-800/60 rounded-lg hidden md:flex"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-950">
        {/* Top Header */}
        <header className="h-16 border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md px-6 flex items-center justify-between z-30 sticky top-0">
          <div>
          ...
          <div className="flex items-center gap-3">
            <Link
              to="/marketing/live-visitors"
              className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/80 hover:border-slate-600 text-xs text-slate-200 transition-all"
            >
              <Radio className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
              <span>{liveCount} Live Sessions</span>
            </Link>

            <Link
              to="/marketing/cart-intelligence"
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/80 border border-slate-700/80 hover:border-slate-600 text-xs text-slate-200 transition-all"
            >
            ...
            {isHead && (
              <Link
                ...
                className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-600/20 border border-rose-500/30 hover:bg-rose-600/30 text-xs font-semibold text-rose-300 transition-all"
              >
