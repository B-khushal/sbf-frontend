...
export const createVendor = async (data: VendorRegistrationData & { userId: string }) => {
  const response = await api.post('/vendors/admin', data);
  return response.data;
};

export const updateVendorById = async (vendorId: string, data: Partial<VendorProfile>) => {
  const response = await api.put(`/vendors/admin/${vendorId}`, data);
  return response.data;
};
