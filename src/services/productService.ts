...
class ProductService {
  ...
  async createProduct(productData: ProductData): Promise<ProductData> {
    ...
    const response = await api.post(`/products`, processedData, config);
  ...
  async updateProduct(id: string, productData: ProductData): Promise<ProductData> {
    ...
    const response = await api.put(`/products/${id}`, processedData, config);
  ...
  async getCategoriesWithCounts(): Promise<{ name: string; count: number }[]> {
    const response = await api.get(`/products/categories-with-counts`);
    return response.data;
  }
...
export const getProducts = async (page = 1, category?: string) => {
  const response = await api.get<{ products: ProductData[], page: number, pages: number }>(`/products`, {
    params: {
      page,
      category,
    },
...
export const getTopProducts = async () => {
  const response = await api.get<ProductData[]>(`/products/top`);
  return response.data;
};
